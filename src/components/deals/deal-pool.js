...
export function DealPool({ pool, onJoin }) {
  ...
  const hasJoined = user ? members.some(member => member.userId === user.id) : false;
  ...
  const handleJoinPool = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "You'll be notified when the minimum group size is reached.",
      });
      ...
      toast({
        ...
        description: 'There was an error joining this deal pool. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setIsJoining(false);
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {members.map((member) => (
              <Avatar key={member.id} className="h-8 w-8" title={`${member.user.firstName} ${member.user.lastName}`}>
            ...
            {Array.from({ length: Math.max(0, pool.minTravelers - members.length) }).map((_, i) => (
      ...
      <CardFooter>
        ...
        >
          {isJoining ? 'Joining...' : 
           ...
           isPoolFull ? 'Pool Full' : 
           isExpired ? 'Expired' : 'Join Pool'}
        </Button>
      </CardFooter>
    </Card>
  );
